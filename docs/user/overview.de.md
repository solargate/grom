...
Folgen Sie anderen Nutzern derselben Instanz und browsen Sie einen gemeinsamen Feed. Unter **Benutzersuche** listet der Bildschirm alle anderen lokalen Benutzer des Servers (mit Folgen/Entfolgen); die Suche nach Nickname oder Federations-Adresse bleibt verfügbar. Liken und kommentieren Sie Workouts von Personen, denen Sie folgen (lokal oder föderiert); Zähler erscheinen in Listen- und Detailansichten.
...
Nur unter **Android** können Sie zusätzlich **Trainings aus Strava importieren** mit eigener Strava-API-Client-ID/Secret aktivieren, einmal per OAuth verbinden und den Sync-Button auf Start nutzen (Anzahl der neuesten Aktivitäten ist auf dem Integrationsbildschirm einstellbar, Standard 10; Stopp bei der ersten bereits importierten). Details: [Strava-API-Import](../integrations/strava-api-import.md).

...
- Profile shows Followers and Following as compact count cards; tap to open the full list
- User search list rows align with the search field for more name space

### Changed

- **UI:** Profile replaces inline follower/following lists with compact count cards; tap opens a dialog or sheet with the list (following count is active only; pending still appear in the list)
